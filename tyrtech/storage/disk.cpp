...
{
    return 0;//m_file.writev(static_cast<uint64_t>(page) << page_bits, iovec, size);
}
  ...
  , m_preallocate_space(preallocate_space)
{
    ...
    //     {
    //         throw exception("{}: invalid data image", m_file.path());
    //     }
    ...
    //     {
    //         throw exception("{}: {}", m_file.path(), system_error().message);
    //     }
